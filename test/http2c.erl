...
wait_for_n_frames(_Pid, StreamId, N, Attempts, Acc)
  when Attempts > 20 ->
    h2_log:error("Timed out waiting for ~p frames on Stream ~p", [N, StreamId]),
    h2_log:error("Did receive ~p frames.", [length(Acc)]),
    h2_log:error("  those frames were ~p", [Acc]),
...
wait_for_n_frames(Pid, StreamId, N, Attempts, Acc) ->
    ...
    case length(Frames) >= N of
        true ->
            h2_log:info("Frames: ~p ~p", [N, Frames]),
...
init([]) ->
    ...
    h2_log:debug("Transport: ~p", [Transport]),
    ...
    h2_log:debug("[client] settings: ~p", [h2_settings:to_proplist(ClientSettings)]),
    ...
    h2_log:debug("AH: ~p, ~p", [AH, PAck]),
    Ack = ?IS_FLAG((AH#frame_header.flags), ?FLAG_ACK),
    h2_log:debug("Ack: ~p", [Ack]),
...
handle_cast(recv, #http2c_state{
    ...
    h2_log:info("http2c recv ~p", [FHeader]),
...
handle_info(Info, State) ->
    h2_log:debug("unexpected [http2c]: ~p~n", [Info]),
    {noreply, State}.
...
process_binary(Bin, Header, <<>>, Frames) ->
    h2_log:info("process_binary(~p,~p,~p,~p", [Bin,Header,<<>>,Frames]),
    L = Header#frame_header.length,
    case byte_size(Bin) >= L of
        true ->
            {ok, Payload, Rem} = h2_frame:read_binary_payload(Bin, Header),
